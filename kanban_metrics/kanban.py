...
def getSingleTaskLeadtime(task_number):
    r = requests.get(url=config.repo_base_url+"issues/" +
                     str(task_number), headers=config.headers)
    if r.status_code == 200:
       ...
       # print(task["title"], task["id"],)
        ...
        if task["closed_at"] == None:
            ...
            closedDate = datetime.strptime(
                task["closed_at"], '%Y-%m-%dT%H:%M:%SZ')
        ...
        for issue in issues:
            if issue["closed_at"] != None:
                ...
                if((closedDate >= startDate) and (closedDate <= endDate)):
                    ...
                    leadtime = (closedDate-opendate).days
                    response.append(
                        "taskNumber: "+str(issue["number"])+" "+"taskTitle: "+issue["title"]+" "+"taskLeadTime: "+str(leadtime)+" days")
...
def getActiveTasks(column_id):
    card_list = []
    r = requests.get(url=config.baseUrl+"projects/columns/" +
                     str(column_id)+"/cards", headers=config.headers)
    if r.status_code == 200:
        data = r.json()
        for cards in data:
